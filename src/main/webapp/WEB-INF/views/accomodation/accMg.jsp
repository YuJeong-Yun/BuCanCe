            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">관리자 DashBoard(숙소예약)</h1>
                    </div>
				<div class="card-body">
							...
							<div class="row">
								<div class="col-sm-12">
									...
									<table class="table table-bordered dataTable" id="dataTable"
										...
										<thead>
											<tr role="row">
												...
												<th class="sorting" tabindex="0" aria-controls="dataTable"
													rowspan="1" colspan="1"
													aria-label="Position: activate to sort column ascending"
													style="width: 87px;">상품명</th>
												<th class="sorting" tabindex="0" aria-controls="dataTable"
													rowspan="1" colspan="1"
													aria-label="Position: activate to sort column ascending"
													style="width: 87px;">체크인</th>
												<th class="sorting" tabindex="0" aria-controls="dataTable"
													rowspan="1" colspan="1"
													aria-label="Position: activate to sort column ascending"
													style="width: 87px;">체크아웃</th>
												<th class="sorting" tabindex="0" aria-controls="dataTable"
													rowspan="1" colspan="1"
													aria-label="Age: activate to sort column ascending"
													style="width: 80px;">결제일</th>
												<th class="sorting" tabindex="0" aria-controls="dataTable"
													rowspan="1" colspan="1"
										...
										<tbody>
												<c:forEach items="${roomMg}" var="roomMg">
											<tr class="even">
												<td class="sorting_1">${roomMg.user_name}</td>
												<td>${roomMg.id}</td>
												<td>${roomMg.roomSort}</td>
												<td>${roomMg.checkIn}</td>
												<td>${roomMg.checkOut}</td>
												<td>${roomMg.accDate}</td>
												<td><button type="button" class="w-btn w-btn-indigo" id="Withdrawal" onclick="deleteCheck('${roomMg.id}');">예약취소</button></td>
											</tr>
											</c:forEach>
										</tbody>
									</table>
...
<script>
		function deleteCheck(id) {
			if (confirm("예약을 취소하시겠습니까?")) {
				...
				document.deleteform.action = "${pageContext.request.contextPath}/accomodation/reCancle";
