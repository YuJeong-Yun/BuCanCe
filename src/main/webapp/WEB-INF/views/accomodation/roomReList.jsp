...
<section class="rooms-section spad">
	<div class="container">
		<div class="row">
			<c:set var="a" value="0" />
			<c:forEach items="${UserPayList}" begin="0"
				end="${UserPayList.size()}">
						...
						<div class="ri-text"
							...
							<table class="table"
								...
								<tbody>
									...
									<tr>
										<td class="r-o">기간</td>
										<td>${UserPayList.get(a).checkIn}~
											${UserPayList.get(a).checkOut}</td>
									</tr>
									...
									<tr>
										...
										<c:if test="${UserPayList.get(a).sort=='ds'}">
											<td class="r-o">이용시간<br>퇴실시간
											</td>
											<td>${UserPayList.get(a).useTime}시간/${UserPayList.get(a).endTime}시
											</td>
										</c:if>
							...
							<form
								action="${pageContext.request.contextPath}/accomodation/roomRefund"
								method="post">
								<input type="hidden" name="accId"
									value="${UserPayList.get(a).accId}"> <input
									type="hidden" name="accName"
									value="${UserPayList.get(a).accName}"> <input
									type="hidden" name="roomSort"
									value="${UserPayList.get(a).roomSort}"> <input
									type="hidden" name="user_name"
									value="${UserPayList.get(a).user_name}"> <input
									type="hidden" name="sort" value="${UserPayList.get(a).sort}">
								<input type="hidden" name="checkIn"
									value="${UserPayList.get(a).checkIn}"> <input
									type="hidden" name="checkOut"
									value="${UserPayList.get(a).checkOut}"> <input
									type="hidden" name="useTime"
									value="${UserPayList.get(a).useTime}"> <input
									type="hidden" name="endTime"
									value="${UserPayList.get(a).endTime}"> <input
									type="hidden" name="accAmount"
									value="${UserPayList.get(a).accAmount}"> <input
									type="hidden" name="accKind"
									value="${UserPayList.get(a).accKind}"> <input
									type="hidden" name="id" value="${UserPayList.get(a).id}">
								<input type="hidden" name="license"
									value="${UserPayList.get(a).license}">


								<c:if test="${UserPayList.get(a).status!='refund'}">
									<button type="submit" class="btn btn-block btn-success btn"
										style="border-top-width: 8; border-bottom-width: 8">
										<b>환불하기</b>
									</button>
								</c:if>
								<c:if test="${UserPayList.get(a).status=='refund'}">
